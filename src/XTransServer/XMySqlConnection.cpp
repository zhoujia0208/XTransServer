#include "StdAfx.h"
#include "XMySqlConnection.h"

XRegistyDataBaseConnectionClass XMySqlConnection::xRegisty("mysql", XMySqlConnection::CreateInstance);
...
XMySqlConnection::XMySqlConnection()
{
...
int XMySqlConnection::Initialize()
{
	//OTL多线程模式
...
int XMySqlConnection::Terminate()
{
...
int XMySqlConnection::Open(const string &strConnectString)
{
	...
	try
	{
		if (!db.connected)
		{
			...
			XLogClass::debug("XMySqlConnection::Open X_SUCCESS");
			return X_SUCCESS;
			//db.set_stream_pool_size(32);
		}
		XLogClass::debug("XMySqlConnection::Open Already");
		return X_SUCCESS;
	}
	catch (otl_exception &ex)
	{
		XLogClass::error("XMySqlConnection::Open OtlException: Message[%s]", ex.msg);
		return X_FAILURE;
	}
}

int XMySqlConnection::Close()
{
	try
	{
		db.logoff();
		XLogClass::debug("XMySqlConnection::Close X_SUCCESS");
		return X_SUCCESS;
	}
	catch (otl_exception &ex)
	{
		XLogClass::error("XMySqlConnection::Close OtlException: Message[%s]", ex.msg);
		return X_FAILURE;
	}
}

int XMySqlConnection::Query(const string &strSql, XDataRow &xDataRow, const int iArraySize, XDataTable &xDataTable)
{
	try
	{
	...
	{
		XLogClass::error("XMySqlConnection::Query OtlException: Message[%s], Text[%s], Info[%s]",
			ex.msg, ex.stm_text, ex.var_info);
		return X_FAILURE;
	}
}

int XMySqlConnection::ExecuteSql(const string &strSql, XDataTable &xDataTable, int64_t &i64RecordCount)
{
	try
	{
	...
	{
		XLogClass::error("XMySqlConnection::ExecuteSql OtlException: Message[%s], Text[%s], Info[%s]",
			ex.msg, ex.stm_text, ex.var_info);
		return X_FAILURE;
	}
};

int XMySqlConnection::BeginTransaction()
{
	try
	{
		db.auto_commit_off();
	...
	{
		XLogClass::error("XMySqlConnection::BeginTransaction OtlException: Message[%s]", ex.msg);
		return X_FAILURE;
	}
}

int XMySqlConnection::Commit()
{
	try
	{
		db.commit();
	...
	{
		XLogClass::error("XMySqlConnection::Commit OtlzException: Message[%s]", ex.msg);
		return X_FAILURE;
	}
}

int XMySqlConnection::RollBack()
{
	try
	{
		db.rollback();
	...
	{
		XLogClass::error("XMySqlConnection::RollBack OtlException: Message[%s]", ex.msg);
		return X_FAILURE;
	}
}

int XMySqlConnection::HeartBeat()
{
	XDataRow xDataRow;
...
int XMySqlConnection::DataTableOTLToX(otl_stream &os, XDataTable &xDataTable)
{
	int iColumnCount = 0;
